...
- All three: unzip, keep the `runtime/` folder, register in the model registry; raw files stay `.gitignore`d (the `curl` lines above become `kawan/scripts/download_models.sh`).

**License handling `[team decision: proceed — risk accepted]`:** official models — app + public-video use is permitted with the Live2D copyright notice; raw files stay out of the repo (`.gitignore` + `kawan/scripts/download_models.sh` hitting the URLs above). LiveroiD — booth terms bar product-asset use; the team proceeds knowingly (noted once, not relitigated; an optional courtesy inquiry to the creator would close it — Q5). Same `.gitignore` treatment + credit lines in README.
