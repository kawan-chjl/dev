...
- **Gotcha:** the SDK example calls `lm.chutes.ai` (single "l"); docs canonically say `llm.chutes.ai`. Both hosts respond `[VERIFIED]`. `[DECISION]` Use `llm.`; day-1 spike confirms the OAuth token against it (fallback `lm.`).
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| Q1  | Does the SIWC access token authenticate against `llm.chutes.ai/v1` exactly (vs `lm.`)?                                                                                    | D1 spike, 1 h                                                        |
| Q2  | Does Pro tier cover all TEE models incl. Kimi-K2.6? (Base tier provably excludes some frontier models)                                                                    | D1 test call + `/users/me/subscription_usage`                        |
| Q3  | Persona/voice register for the hero character (BM/English/Manglish mix?)                                                                                                  | Team taste decision, D2–3                                            |
| Q4  | Does `chutes:invoke` scope authorize `/users/me` for balance display, or is `billing:read` needed?                                                                        | D1 check; affects only the consent-screen scope list                 |
