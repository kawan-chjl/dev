...
- All three: unzip, keep the `runtime/` folder, register in the model registry. `kawan/scripts/download_models.sh` (the `curl` lines above) remains the local-bootstrap path; the runtime files are also committed via **Git LFS** in the private repo `[DECISION — PO-approved deviation, supersedes the "raw files stay `.gitignore`d" rule below]` (see §4.4 license-handling note).

**License handling `[team decision: proceed — risk accepted]`:** official models — app + public-video use is permitted with the Live2D copyright notice. LiveroiD — booth terms bar product-asset use; the team proceeds knowingly (noted once, not relitigated; an optional courtesy inquiry to the creator would close it — Q5). Required credit lines in README either way (`#LiveroiD` hashtag + モデル制作：八城惺架 / @yashiro_seika).

> **`[DECISION — PO-approved deviation from the original "models out of repo / gitignored" rule]`** Model runtime files are now **tracked via Git LFS in the private repo** and served same-origin under `/models/...` (the repo is private, so the original gitignore-for-license-safety motivation no longer applies; license safety still holds via the retained credit lines). `kawan/scripts/download_models.sh` is kept as a local-bootstrap convenience, not the source of truth. The original intent — keep raw files out of a _public_ repo — is preserved here for the record and would re-apply if the repo were ever made public.
...
| DB          | **Supabase Postgres** (SQLAlchemy 2 async + `asyncpg`) for dev+prod; **SQLite (aiosqlite)** for tests + zero-config local fallback `[DECISION — supersedes §7.6-D1's "stay SQLite for the hackathon"]` | Hosted deploy needs a network DB (Render filesystem is ephemeral). One Supabase project serves dev+prod for now via the **Supavisor session pooler (port 5432)**; SQLAlchemy keeps the schema portable. SQLite stays the config default + test DB, never a Supabase mirror. ORM stays SQLAlchemy 2 async — **not Prisma** (Node/TS, incompatible). See §7.6-D1 (revised). |
...
| Hosting     | **Frontend → Vercel** (Vite SPA) · **Backend → Render** (single Web Service, 1 worker) `[DECISION — supersedes the earlier "any single VM / localhost + tunnel" assumption]`                           | A live deployment now exists. Render runs the one FastAPI process (1 worker — APScheduler is in-process, §7.3); Vercel serves the SPA and rewrites `/api/*` → Render for same-origin sessions (Approach A, §7.7). Both auto-deploy from `main` via native Git integration.                                                                                                |
...
POST  /commitments/{id}/abandon             → confirm → missed path; stake fires (§6.3)
...
**D1 — SQLite vs Supabase / managed Postgres → `[REVISED — Supabase Postgres for dev+prod; SQLite retained for tests + local fallback]`.**

> **Revised decision (current):** A live hosted deployment now exists (Render, §7.2/§7.7), and Render's filesystem is ephemeral — the "one box, SQLite on disk" assumption no longer holds for prod. **Persistence is now Supabase Postgres (managed) for both local dev and production**, via the existing **SQLAlchemy 2 async ORM + `asyncpg`** driver — exactly the config swap the original record anticipated. The connection uses the Supabase **Supavisor session pooler (port 5432)** because Render's outbound is IPv4 while Supabase's direct connection is IPv6-only on the free tier. One Supabase project serves dev+prod for now; a Python **seed/reset script (planned under Lane D4)** gives the demo a clean, pre-staged dataset; a second free project for prod isolation is a future option. **SQLite (aiosqlite) is retained** as the test-suite DB and a zero-config local fallback — **not a backup or mirror of Supabase** (they never sync); `tests/conftest.py` hard-forces SQLite so `pytest` can never touch a remote DB, and the config default `database_url` stays SQLite (dev/prod override it via `KAWAN_DATABASE_URL`). ORM stays SQLAlchemy 2 async — **explicitly not Prisma** (Node/TS, incompatible with the Python backend); schema is created via `Base.metadata.create_all`, with **Alembic** as the designated future path if real migrations become necessary.

_Original record (now superseded, kept for rationale):_ The architecture is one long-lived FastAPI process (the WS hub and in-process APScheduler both require it) on one box — SQLite there is zero-infra, zero-latency, and zero external failure modes at demo time; SQLAlchemy makes Postgres a config swap whenever needed. What Supabase would add — hosted Postgres, auth, realtime, storage — duplicates what Kawan already has by design (SIWC auth, its own WS push, evidence files deleted after verdict). It earns its place only if hosting forces an ephemeral filesystem (serverless PaaS — which would also break WS + the scheduler, so we don't host that way). Judges never see the DB; they see whether the demo stalls. _Revisit triggers (now fired): multi-instance deployment, >1 writer process, or a hosted always-on beta after the hackathon._
...
### 7.7 Deployment topology & cross-origin (Approach A) `[DECISION]`

A live deployment now exists (supersedes the §7.2 "production hosting out of scope" assumption).

| Tier           | Platform     | Config                                                                                                                                                                                                                            |
| -------------- | ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Frontend (SPA) | **Vercel**   | Monorepo root `kawan/frontend`; build `bun run build` → `dist/`; SPA rewrite (all routes → `index.html`). Auto-deploys from `main` via native Git integration.                                                                    |
| Backend (API)  | **Render**   | Web Service, root `kawan/backend`, **single instance / 1 worker** (APScheduler is in-process — §7.3); started via `uv run uvicorn app.main:app --host 0.0.0.0 --port $PORT`. Auto-deploys from `main` via native Git integration. |
| Persistence    | **Supabase** | Postgres via Supavisor session pooler (port 5432); §7.6-D1 (revised).                                                                                                                                                             |

Neither tier uses a custom GitHub Actions deploy pipeline — both platforms' native Git integration is the deploy path.

**Cross-origin session — Approach A.** The browser must see the session cookie as same-origin, so:

- Vercel **rewrites `/api/:path*` → Render**, so the browser talks to `/api` same-origin and the session cookie is never cross-site for REST.
- The **WebSocket `/ws` connects directly to Render** (rewrites don't proxy WS) via a frontend `VITE_WS_URL`.
- In production the backend session cookie becomes **`SameSite=None; Secure`** (env-gated via `KAWAN_COOKIE_SAMESITE` / `KAWAN_COOKIE_SECURE`); local dev stays `lax` / insecure.
- CORS allows the Vercel origin via `KAWAN_FRONTEND_ORIGIN`.
...
CREATE TABLE commitments (
  ...
  last_contact_at TIMESTAMP,                             -- Lapse "no contact" signal (§5.3); see docs/adr/0002
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
1. Register once (team `cpk_` key): `POST /idp/apps` with `redirect_uris` (localhost + the **prod Render callback**, now registered) and `allowed_scopes: ["openid","profile","chutes:invoke"]` → store `cid_`/`csc_` in env. The **`chutes:invoke` scope is required** — it covers both `/users/me` balance display and user-billed inference; `billing:read` is privileged and not needed (Q4 resolved). Billing model unchanged: signed-in users are billed to their own Chutes token, guest mode uses the team `cpk_`.
...
6. **Prod secrets** live only in Render's env UI, never the repo: `KAWAN_DATABASE_URL`, `KAWAN_SESSION_SECRET`, an **explicitly-set `KAWAN_FERNET_KEY`** (so encrypted SIWC tokens survive restarts — a generated-per-boot key would silently invalidate stored tokens), the SIWC creds (`cid_`/`csc_`), and the team `cpk_`.
...
| Spike                    | Result                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **S1 SIWC round-trip**   | **PASS `[VERIFIED]`** — app registered via `POST /idp/apps` (`cid_…` + `csc_…` returned once; names must be alphanumeric/space/hyphen/period; `billing:read` requires privileged `unlimited_dev` permission). PKCE flow + token exchange 200 with refresh token. OAuth token → gemma completion **200 on `llm.chutes.ai`**; **`lm.chutes.ai` returns 403** for OAuth tokens — use `llm.` only, `lm.` is NOT a fallback for the user path. **Billing attribution confirmed via dashboard logs** (the OAuth run appears under the consenting account, in a distinct hour-bucket from the 8 `cpk_` runs). _Caveat: consent used the shared team account, so user-vs-team billing **separation** is untested — re-run `kawan/scripts/spikes/s1_siwc_roundtrip.py` with a personal account before the demo, which depends on this separation._ |
