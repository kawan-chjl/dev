...
Language: **MUST** = on the demo thread (spec §12.3), ships or the project fails. **SHOULD** = post-freeze nice-to-have, in stated priority order. **OUT** = roadmap; do not build, may appear on a roadmap slide.
...
Product objectives O1–O5 with demo-visible proofs: spec §0.3.
...
| INV-3 | **Forgiving recovery — no streaks anywhere**       | No streak counters, no chains, no red marks. Misses render as neutral gaps. Skip-days, 6-h grace window, re-scoping, one warm win-back nudge (§2.1, §5.4).                                                                                                                                               |
...
| FR-08 | Hero persona "Kawan" (skeptical concierge, Haru-Receptionist model, gemma-4-31B-turbo-TEE) gets deep tone QA; the two variants (Adik/Hiyori/Qwen3.6, Cik Maid/LiveroiD/DeepSeek-V3.2) ship functional (§11.3). De-scope lever under crunch = variant QA depth, never the picker | MUST (hero) / SHOULD (variant QA depth)  |
...
| FR-10 | Voice input: Web Speech API default; self-hosted Faster-Whisper as URL-flagged mode (§7.2)                                                                                                                                                                                      | MUST (WebSpeech) / SHOULD (Whisper mode) |
| FR-11 | Live2D model assets are NOT committed to the repo: `scripts/download_models.sh` + `.gitignore` + license credits in README (§4.4)                                                                                                                                               | MUST                                     |
...
| FR-24 | GitHub adapter (high trust): no-auth public-repo commits since last check; trivia filter `stats.total < 3` via per-SHA detail (rule visible in UI); author-email setup warning; deterministic pre-checks then one text LLM call relating commits to the deliverable (§10.2) | MUST        |
| FR-25 | Screenshot adapter (medium trust): drag-drop/paste, PNG/JPG/WebP ≤8 MB, client downscale ≤1568 px; judged by TEE vision model with strict JSON schema (verdict/confidence/observations/reasoning/follow_up_request); **file deleted after verdict** (§9.3, §10.3)           | MUST        |
...
| FR-29 | Model routing with inline failover: conversational = `gemma-4-31B-turbo-TEE,Qwen3.6-27B-TEE`; vision judging = `Kimi-K2.6-TEE,Qwen3.5-397B-A17B-TEE`; structured outputs `strict: true` + Pydantic re-validation + one retry (§9.1, §7.6-D2)                                | MUST        |
...
| FR-34 | Idle state (no active commitment): chat disabled except a single "ready to commit?" prompt (§5.5)                                                      | MUST        |
...
| ID    | Requirement                                                                                                                                                                                                                                          | Priority |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------- | ---- |
| FR-36 | State machine exactly per §5.3: `draft → active → (lapsed) → verifying → completed                                                                                                                                                                   | grace    | missed`; `on_track/slipping` are derived flags, not states. Status transitions come from scheduler/verifier only | MUST |
...
| FR-49 | Trust meter — Kawan's skepticism visibly softens per verified win; dips gently on misses; never resets (§11.4)                                                                | SHOULD (#2)             |
...
| NFR-05 | **Compose-to-Start in <60 s** for a returning user (O1)                                                                                                                                                                                               |
...
Escalation (0/1/2) and on_track/slipping are derived, not states. Views V1–V6 + overlays O1/O2: spec §5.5. Canonical week-in-the-life walkthrough incl. alternate paths (miss, lapse, boundary probe, unclear evidence, goalpost move, asleep-at-deadline): spec §6.
...
- Additional evidence adapters beyond GitHub + screenshot (+ optional URL probe): no Google Docs, Notion, Strava, YouTube, Todoist, etc. Never promise the dead ones: Google Fit, Goodreads, Instagram/TikTok, HealthKit (§10.4)
...
Acted-on assumptions A1–A8 (skip-days=1, grace=6 h, trivia<3, intake≤6 Q, screenshot≤8 MB, SMTP/Resend, SQLite, Docker voice on one machine): spec §14.
