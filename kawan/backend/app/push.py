...
def _send_one(subscription: dict, headline: str, url: str = "/home") -> tuple[bool, bool]:
    """Return (delivered, is_gone). is_gone=True when the push service signals the sub is dead.
    headline is the visible notification body (privacy/iOS, TR-17); url rides as data.url
    so the service worker can open the right workspace on click."""
    from pywebpush import WebPushException, webpush  # imported lazily so the dep is optional in dev
    try:
        webpush(
            subscription_info=subscription,
            data=json.dumps({"headline": headline, "url": url}),
...
async def push_to_user(db: AsyncSession, user_id: str, headline: str, url: str = "/home") -> bool:
    """Returns True if at least one subscription accepted the push.
    Prunes subscriptions the push service reports as gone (404/410).
    url is the deep-link included as data.url in the push payload (plan §4.4)."""
    ...
    results = await asyncio.gather(
        *(asyncio.to_thread(_send_one, s.subscription, headline, url) for s in subs)
    )
